...
WORKDIR /src
COPY ["little tasks.csproj", "little tasks/"]
RUN dotnet restore "little tasks/little tasks.csproj"

WORKDIR "/src/little tasks"
COPY . .
RUN dotnet build "little tasks.csproj" --configuration Release --output /app/build

FROM build AS publish
RUN dotnet publish "little tasks.csproj" \
...
Execute unit tests
```
cd "little tasks/test/little tasks.Tests"
dotnet test
```

Deploy function to AWS Lambda
```
cd "little tasks/src/little tasks"
